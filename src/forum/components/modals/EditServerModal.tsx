...
const formSchema = z.object({
  ...
  image: z
    ...
    .refine((file) => file instanceof File && file.size > 0 && file !== null, {
      message: 'Please upload an image file.',
    })
    .or(z.string().min(1, 'Image 64 not loaded')),
});
...
export default function EditServerModal() {
  ...
  useEffect(() => {
    if (server) {
      ...
      setValue('image', server.imageUrl ?? null);
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    ...
    try {
      await axios.patch(
        `${config.APP_BASE_URL}/server/edit-server/${server?.id}`,
        formData,
        configHeader
      );
  ...
  const imageSrc: string | undefined =
    typeof file === 'string' ? `data:image/jpeg;base64,${file}` : file?.preview;
